...
const materialsModalSubtitle = document.getElementById(
  "materialsModalSubtitle",
);
const materialsModalBody = document.getElementById("materialsModalBody");
const presentationToggleHome = document.getElementById(
  "presentationToggleHome",
);
...
const materialsPageContainer = document.getElementById(
  "materialsPageContainer",
);
...
function renderBreadcrumbs(subject, theme) {
  ...
  const depth = theme
    ? "../../"
    : subject && !materialsModalEl?.closest("body")
      ? "../"
      : "";

  // Correcció depth per a index.html principal
  const isMainIndex =
    !materialsPageContainer && !subjectIndexContainer && !themeIndexContainer;
...
async function loadSubjects() {
  ...
  if (dataFound) {
    ...
    console.error(
      "No s'ha trobat el fitxer subjects.json a cap de les rutes provades.",
    );
...
function renderSubjectIndex(subject) {
  ...
  if (
    ...
    subjectIndexContainer.innerHTML =
      '<div class="text-muted">No hi ha materials.</div>';
    return;
  }
  ...
  function themeMatches(section, query, filterTag) {
    ...
    return (
      queryOk && section.title.toLowerCase().includes(filterTag.toLowerCase())
    );
  }
  ...
  function renderSubjectThemes(
    query = "",
    filterTag = "",
    sortMode = "",
    viewMode = "detailed",
  ) {
    // Ordenem per camp `sa` si està present; després apliquem l'ordenació triada
    const raw = subject.materials
      .filter((section) => themeMatches(section, query, filterTag))
      .slice();
    raw.sort((a, b) => {
      const as = typeof a.sa === "number" ? a.sa : Infinity;
      const bs = typeof b.sa === "number" ? b.sa : Infinity;
      if (as !== bs) return as - bs;
      return (a.title || "").localeCompare(b.title || "", "ca");
    });

    const filtered = sortThemes(raw, sortMode);
    ...
    const cards = filtered
      .map((section) => {
        ...
        const buttonClass = isCompact
          ? "btn btn-outline-primary btn-sm w-100 btn-modern"
          : "btn btn-primary btn-sm w-100 btn-modern";
        const slug = section.slug || "";
        const href =
          section.directHref || (slug ? `./${slug}/index.html` : "#");
        return `
          <div class="${colClass}">
            <article class="card h-100 border-0 shadow-sm subject-card theme-card ${isCompact ? "theme-card-compact" : ""}">
              <div class="card-body ${bodyPaddingClass}">
                <div class="d-flex justify-content-between align-items-start gap-2 mb-2">
                  <h2 class="${titleClass} mb-0">
                    ${section.title}
                    ${section.sa ? `<span class="badge text-bg-secondary ms-2">SA ${section.sa}</span>` : ""}
                  </h2>
  ...
  const topicFilters = (subject.topics || [])
    .map(
      (topic) =>
        `<button type="button" class="btn btn-outline-primary btn-sm btn-modern js-theme-filter" data-filter="${topic}">${topic}</button>`,
    )
    .join("");
  ...
  const filterButtons =
    subjectIndexContainer.querySelectorAll(".js-theme-filter");
...
function renderThemeIndex(subject, themeSlug) {
  ...
  if (!section) {
    // Si no hi ha secció en subjects.json, intentem detectar fitxers locals coneguts
    generateLocalThemeIndex([
      "apunts-html.html",
      "apunts-css.html",
      "apunts-javascript.html",
      "bootstrap-teoria.html",
    ])
      .then((html) => {
        themeIndexContainer.innerHTML =
          html || '<div class="text-muted">Tema no trobat.</div>';
      })
      .catch(() => {
        themeIndexContainer.innerHTML =
          '<div class="text-muted">Tema no trobat.</div>';
      });
    return;
  }
...
async function generateLocalThemeIndex(candidates) {
  if (!Array.isArray(candidates) || candidates.length === 0) return "";
  const checks = await Promise.allSettled(
    candidates.map(async (file) => {
      try {
        const resp = await fetch(file, { method: "GET" });
        return resp.ok ? { file, ok: true } : { file, ok: false };
      } catch (e) {
  ...
  const listItems = existing
    .map(
      (f) =>
        `<li><a class="link-body-emphasis" href="${f}">${formatTitleFromFilename(f)}</a></li>`,
    )
    .join("");

  return `
    <div class="mb-3">
      <h1 class="h4 mb-2">Continguts del tema</h1>
...
function formatTitleFromFilename(fn) {
  return fn
    .replace(/\.html?$/i, "")
    .replace(/-/g, " ")
    .replace(/\b(apunts)\b/i, "Apunts")
    .replace(
      /\b(bootstrap teoria|bootstrap_teoria|bootstrap-teoria)\b/i,
      "Bootstrap · Teoria",
    )
    .replace(/\b(html|css|javascript)\b/i, (m) => m.toUpperCase());
}
...
function filterSubjects() {
  ...
  subjects.forEach((subject) => {
    const subjectMatches =
      `${subject.name} ${subject.description} ${subject.topics.join(" ")}`
        .toLowerCase()
        .includes(query);
      ...
      subject.materials.forEach((section) => {
        if (section.title.toLowerCase().includes(query)) {
          ...
          const href =
            ...
            (section.slug
              ? subjectPage
                ? `${subjectPage.replace("index.html", "")}${section.slug}/index.html`
                : `#`
              : "#");
          matchedItems.push({
            type: "Tema",
...
function renderSearchResults(subjectsList, itemsList, query) {
  ...
  if (subjectsList.length > 0) {
    ...
    h.className = "col-12 mb-3";
    h.innerHTML = `<h3 class="h6 text-uppercase text-muted">Assignatures</h3>`;
  ...
  if (itemsList.length > 0) {
    const h = document.createElement("div");
    h.className = "col-12 mt-4 mb-3";
    h.innerHTML = `<h3 class="h6 text-uppercase text-muted">Temes i materials</h3>`;
    subjectsContainer.appendChild(h);
...
function initPresentationModeHome() {
  if (presentationToggleHome) {
    presentationToggleHome.addEventListener("click", () => {
      ...
      presentationToggleHome.textContent = active
        ? "Sortir mode aula"
        : "Mode pantalla aula";
  ...
  document.addEventListener("fullscreenchange", () => {
    ...
    fullscreenToggleHome.textContent = document.fullscreenElement
...
document.addEventListener("click", (e) => {
  ...
  if (btn)
    openMaterials(decodeURIComponent(btn.getAttribute("data-subject") || ""));
  ...
  if (
    ...
    if (searchInput) {
      ...
      if (subjectThemeSearch) {
        ...
        subjectThemeSearch.scrollIntoView({
          behavior: "smooth",
          block: "center",
        });
      }
